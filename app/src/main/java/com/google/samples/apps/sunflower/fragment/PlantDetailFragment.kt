...
class PlantDetailFragment : BaseFragment() {
	...
	private val plantDetailViewModelFactory = object : PlantDetailViewModel.AssistedFactory {
		override fun create(plantId: String): PlantDetailViewModel {
		...
		val binding = DataBindingUtil.inflate<FragmentPlantDetailBinding>(
			...
			callback = Callback { plant ->
				plant?.let {
					...
					Snackbar.make(root, string.added_plant_to_garden, Snackbar.LENGTH_LONG)
						.show()
				}
			}
	...
	private fun createShareIntent() {
		val shareText = plantDetailViewModel.plant.value.let { plant ->
			if (plant == null) {
				...
				getString(string.share_text_plant, plant.name)
			}
		}
